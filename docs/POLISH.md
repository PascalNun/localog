...
The current single heavy-work lane is a useful safety boundary. Memory recommendations are partly there — the machine's memory reaches the settings view and shapes what is recommended, on macOS. First-run setup is **half built as of 25 August 2026**. The start screen now says that a new installation has no transcription model, what it costs, and offers the download where the first decision is made rather than leaving it in Settings; it appears only when nothing is installed, and it is not a gate. The recording permission followed the same day: the recorder answers a `--check` without creating anything, and the record screen asks before the button rather than inferring it from twelve seconds of silence afterwards, with a button that opens the System Settings pane and a re-check when the window comes back. What is still missing: choosing a quality other than the default means going to Settings and knowing to, and nothing yet introduces speaker separation, which needs two more models.
