...
| D-029 | Automatic speaker diarisation is a v0.1 capability via an ONNX diariser (sherpa-onnx), not a Python/PyTorch stack; speakers stay editable and provisional | Accepted direction; runtime/quality spike-gated | Speakers were always intended; ONNX keeps it bundleable and avoids the Python footprint already rejected. Implementation gated on a diarisation spike                                   |
