...
4. The protocol is the outcome. Transcription is a reviewable source, not the final product.
   4a. **Nothing the application does to a person's material is destructive.** Every step keeps what it started from, and every change can be seen and undone. This is already how the product is built rather than something to work towards: imported originals are copied and never modified; a recording's trims and removals are a description of what to use, not a cut to the audio; the transcript working copy is separate from immutable revisions; protocol revisions are retained and a review belongs to one exact revision; speaker labels stay provisional until a person names them; corrections are shown in context before they apply and can be applied to some occurrences and not others; exports never silently overwrite; and a protocol with an unreadable stretch names it rather than closing over the hole.

   The consequence worth stating, because it decides interface questions: **it is what makes a confident default safe.** Offering to remember a corrected spelling for the whole project, ticked by default, would be reckless in a system where being wrong costs something; here undoing it is one click on the Names & terms screen. Where a mistake is cheap to reverse, the interface should do the useful thing without asking. Where it is not, it must ask — which is why sending, publishing, and deleting are different in kind and stay explicit.
