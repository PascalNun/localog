...
| D-014 | Phase 0 discovers installed or user-provided models only; no model-download manager                                                                       | Superseded by D-028                                                  | Sequencing choice to validate the workflow first; the intended product always managed models for the user                                                                               |
...
| D-022 | `whisper.cpp` is the first candidate for the distributable transcription runtime                                                                          | Accepted (see D-028)                                                 | Command/JSON/`--version` contract validated against real whisper.cpp v1.9.2 with Metal on 2026-08-04; bundled per D-028                                                                 |
...
| D-028 | Bundle the whisper.cpp binary as a signed sidecar; deliver transcription/diarisation models by consent-gated on-demand download into app-managed storage  | Accepted                                                             | The user chooses a quality outcome, not a runtime; manual executable/model paths were dev scaffolding. Supersedes D-014. See [TRANSCRIPTION_EXPERIENCE.md](TRANSCRIPTION_EXPERIENCE.md) |
| D-029 | Automatic speaker diarisation is a v0.1 capability via an ONNX diariser (sherpa-onnx), not a Python/PyTorch stack; speakers stay editable and provisional | Accepted; runtime validated by spike, quality evidence still limited | Speakers were always intended; ONNX keeps it bundleable and avoids the Python footprint already rejected. Implementation gated on a diarisation spike                                   |
...
1. Transcription runtime/distribution is settled for the local path (D-028: bundled whisper.cpp sidecar + on-demand models). The open part is the **protocol-generation** provider: Ollama is approved only for spikes and early technical previews, and a self-contained generation runtime remains undecided.
...
## Speaker diarisation spike result

The isolated study under `spikes/speaker-diarisation/` tested the ONNX direction accepted in D-029,
on 2026-08-05, on the M1 Pro/16 GB development machine rather than the M1/8 GB baseline.

What was tested: prebuilt `sherpa-onnx` v1.13.4 macOS arm64 binaries with the
`pyannote-segmentation-3-0` segmentation model and a 3D-Speaker embedding model, against a synthetic
three-speaker German meeting generated from macOS voices with exact ground truth. No real meeting
material was used.

Measurements on a 23.5-second fixture:

- 7.24 s wall time, approximately 3.2 times faster than real time.
- 259 MB peak resident memory.
- 46 MB of added model footprint (7.2 MB segmentation, 39 MB embedding).
- 88.2 % frame accuracy at 10 ms when the speaker count was supplied; all three speakers separated
  and consistently identified, with one 0.56 s misattribution.
- Automatic speaker-count detection returned the correct count at every threshold from 0.4 to 0.8,
  so neither the user nor a per-recording tuning step needs to supply it.

Keep/change decision:

- **Keep** `sherpa-onnx` as the candidate diariser behind an application port, and keep the fixture
  and its ground truth as a test oracle.
- **Change before implementation:** compare a multilingual or German-suited embedding model against
  the Chinese-trained one used here; test overlapping speech and a long recording; repeat on the
  M1/8 GB baseline.
- **Retain the spike as a reference only.** The production adapter must use the accepted supervised
  process boundary with cancellation and bounded output, as the whisper.cpp adapter does.

Risks and limits: synthetic speech is unrealistically clean and acoustically distinct, no overlapping
speech was tested, the embedding model is trained on Chinese, only one short fixture was measured, and
alignment between diariser turns and whisper segments — the substantive remaining design problem —
was not attempted.

Packaging note: the downloaded binaries are killed by macOS on Apple Silicon (`SIGKILL`, exit 137,
with no output at all) until ad-hoc signed. Shipped binaries are signed with the application, so this
affects development rather than distribution, but the silent failure mode is worth recording.

## Deferred decisions

- Portable project-bundle format beyond the v0.1 backup/restore consideration.
- Integrated microphone/system-audio recording (still Phase 2).
- Public provider/plugin SDK and broad provider capability negotiation.

Automatic model acquisition and automatic diarisation are **no longer deferred** — they are accepted as v0.1 direction under D-028 and D-029, gated by their validation spikes.
