...
**The style instruction did not work.** Adding "a speaker label is not a person's name; describe them by role where no name was said" and measuring at three seeds: the fault persisted at two of them, in new formats — `**Elektroplanung (NOKERA):** Speaker 1` at one, `- Speaker 1 (Elektroplanung)` at another — and the third had never shown it, so it evidenced nothing either way. Figures were unaffected at 30 to 31 of 39, so the instruction was not harmful, merely useless: it supplies no information the model was missing.
