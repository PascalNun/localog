...
pub(crate) fn run_job(
    ...
    match result {
        ...
        Err(ProcessingError::Cancelled) => {
            finish_non_success(root, job_id, "cancelled", "cancelled", None, &notify)?;
        ...
        Err(error) => {
            ...
            finish_non_success(root, job_id, "failed", "failed", Some(code), &notify)?;
...
fn execute_transcription(
    ...
    let artifact = adapter.transcribe(
...
fn finish_non_success(
    ...
    remove_staged(root, &job);
    repository.connection.execute(
        "UPDATE jobs SET state = ?1, stage = ?2, error_code = ?3,
                updated_at_ms = ?4, finished_at_ms = ?4 WHERE id = ?5 AND state != 'completed'",
        params![state, stage, error_code, unix_time_millis(), job_id],
    )?;
    notify(repository.workspace_snapshot()?);
    Ok(())
}
